...
def train_model(model, loss_fn, optimizer, loader, scaler, max_epoch, device, gene_mask):
    # Ensure gene_mask is on the correct device
    ...
    for epoch in range(max_epoch):
        ...
        for batch in loader:
            ...
            with autocast():
                ...
                # Restrict loss computation to gene-containing nodes in the batch
                batch_gene_mask = gene_mask[batch.n_id]  # Map global gene_mask to batch nodes
                valid_mask = batch.train_mask & batch_gene_mask  # Combine with batch's train_mask
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            with autocast():
                ...
                # Restrict predictions and labels to gene-containing nodes in the batch
                batch_gene_mask = gene_mask[batch.n_id]  # Map global gene_mask to batch nodes
                valid_mask = batch.test_mask & batch_gene_mask
...
def main():
    ...
    gat = GAT(in_channels=num_feat, hidden_channels=[10, 50]).to(device)
